...
public class ImageTool {
    ...
    public static void loadImageWithUrl(Context context, String url, ImageView imageView) {
        //不要placeholder
        GlideApp.with(context).load(url).skipMemoryCache(true)/*.placeholder(R.drawable.default_small)*/.into(imageView);
    }

    public static void loadCircleImageWithUrl(Context context, String url, ImageView imageView) {
        //不要placeholder
        GlideApp.with(context).load(url).skipMemoryCache(true).transform(new GlideCircleTransform())/*.placeholder(R.drawable.default_small)*/.into(imageView);
    }

    public static void loadImgByGlideToImageView(Context context, String imgUrl, int placeHolderRes, ImageView imageView) {
        //不要placeholder
        GlideApp.with(context).load(imgUrl).skipMemoryCache(true)/*.placeholder(placeHolderRes)*/.into(imageView);
    }

    public static void loadImgByGlide(Context context, String imgUrl, int placeHolderRes) {
        GlideApp.with(context).load(imgUrl).skipMemoryCache(true)/*.placeholder(placeHolderRes)*/;
    }

    public static void loadImgByGlide(Context context, String imgUrl) {
        GlideApp.with(context).load(imgUrl).skipMemoryCache(true);
    }

    public static void loadByGlide(Context context, String url, ImageView imageView) {
        RequestOptions options = new RequestOptions().error(R.drawable.place_holder_song).placeholder(R.drawable.place_holder_song);
